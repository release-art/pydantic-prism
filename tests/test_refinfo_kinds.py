...
def test_via_and_scope_are_not_on_other_kinds() -> None:
    ...
    id_info = Order.__refs__["customer_id"]
    assert not hasattr(id_info, "via")
    assert not hasattr(id_info, "scope")
    back_info = Customer.__refs__["order_ids"]
...
def test_key_type_and_many_stay_on_base() -> None:
    ...
    dict_ref = Order.__refs__["highlights"]
    ...
    assert Order.__refs__["customer_id"].many is False


def test_accessors_return_precise_subtypes() -> None:
    assert all(isinstance(v, IdRefInfo) for v in Order.__refs__.outgoing.values())
    assert all(isinstance(v, BackRefInfo) for v in Customer.__refs__.incoming.values())
    assert all(isinstance(v, EmbeddedRefInfo) for v in Order.__refs__.embedded.values())


def test_subtypes_survive_projection() -> None:
    proj_refs = Order.scope(Public).__refs__
