...
| `Model.tool_schema(scope=None, *, provider="openai", strict=True, name=None, description=None, envelope=True)` | classmethod | One-step LLM tool / function schema for a scope — `Model.scope(scope).tool_schema(...)`. `scope` falls back to `default_scope=`. See [`Projection.tool_schema`](#projection--derived-classes) and [derive an LLM tool schema](../how-to/derive-llm-tool-schema.md). |
...
(2) keep `mode="before"` and call `cls.run_inherited_before(data)` at the top to
run the inherited hooks first (they re-run under pydantic, so must be
idempotent — the explicit call makes that double-run visible); (3) if it doesn't
depend on the base hook, `parent_ordering="acknowledged"`. See
